...
1. **Сканирование входа**
   ...
   - Для каждого CSV вычисляет `SHA-256`, размер и тип сущности.
...
- `_resolve_entity(file_name: str) -> str`
  - Определение сущности по префиксу имени.
...
- `stands` — список стендов;
- `entities` — префикс файла и `business_key` каждой сущности;
