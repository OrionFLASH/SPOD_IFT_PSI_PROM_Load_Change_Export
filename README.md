...
4. **Объединение между стендами**
   ...
   - Колонка **`source_stands`** строится по **фактическому совпадению всех полей выводимой строки** с данными на стенде для того же `business_key` (а не только по принадлежности к партиции merge): в список попадают только стенды, где в CSV реально лежит **хотя бы одна** строка с теми же значениями, что показаны в отчёте (эталон значений — `merge.reference_row_stand`, по умолчанию `PROM`). Если в одном файле **несколько строк с одним и тем же `business_key`** (дубликат ключа), стенд попадёт в `source_stands`, если совпадает **любая** из них — при парсинге пишется предупреждение в лог.
