...
| Миграции SQLite | **[частично]** (есть авто-расширение схемы на старых БД, нет отдельного migration-framework) |
...
3. **Дедупликация файлов и история актуальности в SQLite**
   ...
   - В `raw_rows` дополнительно сохраняется `file_hash_sha256` для связки с историческими данными.
   - Для каждой пары `stand+entity` создается отдельная историческая таблица `csv_<entity>_<stand>`:
     - хранит полный снимок строк исходного CSV (`row_json`) и технические поля (`row_hash`, `business_key`, `row_num`);
     - поддерживает историю актуальности (`status`, `is_actual`, `first_seen_at`, `last_seen_at`, `last_status_changed_at`);
     - не допускает повторной записи того же файла за счет уникальности `(file_hash_sha256, row_num)`.

4. **Исторические таблицы `csv_*` и ссылки на первичные строки**
   - Таблицы `csv_<entity>_<stand>` формируются динамически на старте (`_init_db`) для всех сущностей и всех стендов из конфига.
   - Для текущего запуска:
     - предыдущие актуальные строки того же `stand+entity` переводятся в `HISTORICAL`;
     - строки актуального файла получают `ACTUAL`;
     - при повторной встрече того же `file_hash_sha256` вставка не выполняется, обновляются только признаки актуальности и времена.
   - Для связывания с аудит-таблицами кэшируется PK каждой строки: `(entity, stand, file_hash_sha256, row_num) -> id`.

5. **Объединение дублей по ключу**
...
7. **Аудит сравнений в основной БД (`DIFF`/`CONS`)**
   - Добавлены служебные таблицы:
     - `comparison_sets` — уникальные наборы сравниваемых файлов (сигнатура, payload, время создания/последнего запуска);
     - `comparison_set_files` — состав конкретного набора (`stand`, `entity`, `file_name`, `file_hash_sha256`, `file_path`);
     - `diff_records` — детальные расхождения по дублям (аналог `DIFF_REPORT`) с ссылками на строки в `csv_*`;
     - `cons_records` — нарушения консистентности (аналог `CONS_REPORT`) с ссылками на строки в `csv_*`.
   - В `diff_records`/`cons_records` сохраняются диагностические поля:
     - `diff_columns`, `diff_positions`, `diff_snippets`.
   - Реализована дедупликация повторных сравнений:
     - если набор файлов уже сравнивался (совпала сигнатура `comparison_sets.signature_hash`), повторная запись `DIFF/CONS` в БД не выполняется;
     - при этом Excel-отчет продолжает формироваться как обычно.

8. **Консольный отчет процесса**
...
- `_init_db(conn)`
  - Создание таблиц `runs`, `ingested_files`, `raw_rows`, `merged_rows`, `comparison_sets`, `comparison_set_files`, `diff_records`, `cons_records`, а также `csv_<entity>_<stand>`.
...
- `_persist_csv_snapshot(conn, descriptor, parsed_rows) -> None`
  - Запись исторического снимка исходного CSV в `csv_<entity>_<stand>` с учетом `ACTUAL/HISTORICAL`.
- `_upsert_comparison_set(conn, files) -> tuple[int | None, bool]`
  - Регистрация набора сравнения файлов и определение, новый он или уже был.
- `_persist_diff_and_cons_records(conn) -> None`
  - Запись `DIFF/CONS` в основную БД только для нового набора сравнения.
...
- **[сделано]** Реформа основной БД: добавлены исторические таблицы `csv_<entity>_<stand>` с `ACTUAL/HISTORICAL`, таблицы аудита `comparison_sets`, `comparison_set_files`, `diff_records`, `cons_records` и связи `DIFF/CONS` с первичными CSV-строками через PK.
- **[сделано]** Добавлена дедупликация повторных сравнений: при уже известном наборе файлов `DIFF/CONS` в БД повторно не пишутся.
